 ...
 */
using NodeFactory =
    std::function<Core::Node*(Core::Graph&, const std::string&, const nlohmann::json&)>;
  ...
  static const std::map<std::string, NodeFactory> factories = {
      {"RandomDataNode",
       [](Core::Graph& graph, const std::string& nodeName,
         ...
         int samples =
             params.contains("samples") ? params["samples"].get<int>() : kDefaultSamples;
      ...
      {"SineNode",
       [](Core::Graph& graph, const std::string& nodeName,
         ...
         double amplitude = params.contains("amplitude")
                                ...
                                : kDefaultAmplitude;
         double phase = params.contains("phase") ? params["phase"].get<double>()
                                                  : kDefaultPhase;
         double fs =
      ...
      {"FilterNode",
       [](Core::Graph& graph, const std::string& nodeName,
         ...
         auto mode = params.contains("filter_mode")
                         ? static_cast<Filter::Mode>(
                               params["filter_mode"].get<int>())
                         : kDefaultFilterMode;
         auto type = params.contains("filter_type")
                         ? static_cast<Filter::Type>(
                               params["filter_type"].get<int>())
                         : kDefaultFilterType;
         ...
         return graph.createNode<FilterNode>(nodeName, mode, type, order,
                                             cutoffFreq, samplingFreq);
       }},
...
Core::Graph loadFromJson(const std::string& jsonString) {
  ...
  try {
    ...
    for (const auto& nodeJson : j["nodes"]) {
      ...
      nlohmann::json params = nodeJson.contains("parameters") ? nodeJson["parameters"]
